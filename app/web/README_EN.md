...
- `@nuxtjs/i18n`, strategy `prefix` (locale-prefixed routes), default `zh-cn`. 18 locales (`zh-cn`, `zh-tw`, `en-us`, `ja`, `ko`, `ru`, `uk`, `pl`, `de`, `fr`, `it`, `es`, `pt`, `tr`, `id`, `vi`, `th`, `ar`) plus hidden `zh` / `en` aliases for legacy links
...
- RTL: RTL locales declare `dir: 'rtl'` in `i18n.locales` (`ar` does), and `app/components/MiRoms.vue` emits `<html lang dir>`. Keep `<MiRoms />` **outside** the `ClientOnly` block in `app.vue`: inside it, the attributes never reach the SSR HTML, so RTL locales render LTR first and then flip (visible direction flash). Direction-sensitive styling uses logical utilities (`ps-*`/`pe-*`, `ms-*`/`me-*`, `start-*`/`end-*`, `text-start`, `border-e`), and directional icons carry `dir-flip` (mirrored via `[dir='rtl'] .dir-flip { scale: -1 1 }` in `main.css`)
